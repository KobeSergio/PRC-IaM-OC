...
export default function Dashboard() {
  ...
  const defaultInspections = inspections.filter(
    (inspection) =>
      inspection.inspection_task.includes("inspection approval") ||
      inspection.inspection_task.includes("cancellation approval")
  );
  ...
  useEffect(() => {
    if (filteredInspections.length > 0) {
      ...
      const _inspectionApproval = filteredInspections.filter((inspection) =>
        inspection.inspection_task.includes("inspection approval")
      ).length;
      const _cancellationApproval = filteredInspections.filter((inspection) =>
        inspection.inspection_task.includes("cancellation approval")
      ).length;

      setInspectionApproval(_inspectionApproval);
      setCancellationApproval(_cancellationApproval);
            ...
            <div className="w-full lg:w-1/2 bg-white border border-[#D5D7D8] flex flex-col lg:flex-row rounded-[10px] p-6 gap-5">
              <div className="w-full flex flex-col">
                ...
                <h3 className="font-monts font-semibold text-base text-darkerGray">
                  {inspectionApproval + cancellationApproval} total,{" "}
